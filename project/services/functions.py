...
def colombia_stats():
    return {
        "Net Loan Portfolio in Banking Establishments": "533,509,840*",
        "Net Loan Portfolio in Other Financing Companies": "11,802,396*",
        "Coverage": "163.2%",
...
def plot_stat_two(df: pd.DataFrame, y: str, normalize: Union[bool, str] = False) -> Figure:
    c_table = contigency_table(
        df[CategoricalFeature.LOAN_STATUS.name.lower()], df[y], normalize)
...
def map_form_to_one_hot_encoding(inputs: list, form_features: List[Enum]) -> Dict:
    ...
    for i in range(len(inputs)):
        encoded_feature_input = map_feature_to_one_hot_encoding(
            inputs[i], form_features[i])
